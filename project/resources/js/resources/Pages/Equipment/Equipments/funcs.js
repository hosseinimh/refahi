...
import { equipmentsPage as strings } from "../../../../constants/strings";
import { Equipment as Entity } from "../../../../http/entities";
import {
    setLoadingAction,
    setPagePropsAction,
    setTitleAction,
...
const addAction = () => {
    _navigate(`${basePath}/equipments/add`);
};

const editAction = (item) => {
    if (!isNaN(item?.id) && item?.id > 0) {
        _navigate(`${basePath}/equipments/edit/${item.id}`);
    }
};
...
const fillForm = async (data = null) => {
    ...
    await fetchEquipments(data);

    _dispatch(setLoadingAction(false));
};

const fetchEquipments = async (data = null) => {
